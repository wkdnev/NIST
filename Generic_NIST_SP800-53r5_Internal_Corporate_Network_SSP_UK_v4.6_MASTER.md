GENERIC INTERNAL CORPORATE APPLICATION  
...
Document status: Controlled corporate security standard - MASTER COPY  
Standard identifier: GICA-SSP-001  
Version: 4.6  
Date: 22 July 2026  
Classification/handling: Organisation Controlled - Security Documentation  
Editing status: The normative body of this document must not be altered for individual applications.

This document describes a defensible standard implementation pattern. It is not an authorisation decision and does not, by itself, establish compliance. Each deployed application must validate applicability, inherited controls, parameter values, evidence, data categories, and legal/contractual obligations.
...
| 4.6         | 24 Jul 2026 | Reworked AU-02 Event Logging to distinguish inherited enterprise logging for identity, Windows EUC, VPN, network, hosting, managed platforms, PAM, time and SIEM services from the minimum application-specific event-selection and generation requirements. Added an application logging matrix, risk-based event categories, structured content, cross-tier correlation, sensitive-data minimisation, central forwarding, failure handling, protection, testing, evidence, monitoring measures, related-control explanation and explicit addendum triggers. Added reference to NIST SP 800-92 and noted the draft status of Revision 1.                                       | Enterprise Security Architecture |
...
![Figure 1. Generic internal corporate-network application reference architecture](Figure_1_Generic_Internal_Corporate_Network_Architecture.png)
...
MASTER TEXT CONTROL: Do not alter any control statement, architecture-specific application, evidence expectation, measure or addendum trigger below. Record every application-specific difference exclusively in the Application Addendum.
...
2.  Include sufficient context. Each event must include an accurate timestamp derived from the enterprise time service, the user or service identity, the application/component, the action attempted or completed, the outcome, and a suitable object, record or transaction reference. Logs must not contain passwords, authentication secrets, cryptographic keys or unnecessary restricted data.

3.  Send events to the enterprise monitoring service. Security-relevant events must be forwarded using an approved, reliable mechanism to the enterprise log collector or SIEM. Where direct forwarding is not technically possible, an approved intermediary or agent must be used. Loss of logging or repeated forwarding failure must be detectable.

4.  Define the application monitoring profile. The application owner must maintain a concise logging and monitoring matrix identifying required event types, event sources, log destinations, responsible owners, retention classification and any application-specific alert conditions. Application-specific alerts should be limited to credible misuse or compromise scenarios that enterprise infrastructure monitoring cannot identify without business context.

5.  Support investigation and tuning. The application support team must assist the SOC or incident-response team by interpreting application events, identifying expected behaviour, validating suspicious activity and preserving relevant application evidence. It must also correct excessive, missing or misleading logging through the normal change process.

6.  Verify monitoring after material change. Logging and forwarding must be tested when the application is first onboarded, after material changes to authentication, authorisation, interfaces, data flows or logging components, and periodically under the continuous-monitoring schedule.
...
AU-02 requires the organisation to identify the event types that systems must log, explain why those events are needed, coordinate the selection with relevant operational, security, privacy, legal and records functions, and periodically review and update the selection. The purpose is to create useful, proportionate and defensible event records that support accountability, security monitoring, incident response, operational diagnosis, control assessment and authorised investigations. AU-02 determines what is to be logged; related controls govern the content of records, generation, time synchronisation, protection, review, retention and response. In this corporate model, the enterprise defines the common event-logging standard and collects events from corporate identity, Microsoft Windows EUC, VPN, network, server, database, platform, privileged-access and security services. Each application remains responsible for identifying the security-relevant events produced by its own business functions, roles, interfaces, data handling and application-controlled components, enabling those events, and providing enough context for central analysis without recording unnecessary sensitive information. This implementation reflects AU-02 and the corresponding assessment objectives in NIST SP 800-53 and SP 800-53A, supported by the log-management principles in NIST SP 800-92 \[1\]\[2\]\[12\].

### Inherited enterprise event-logging controls

The following capabilities are inherited when the application uses approved corporate identity, EUC, VPN, network, hosting, database, platform, PAM and security-monitoring services. They are implemented and evidenced by the relevant enterprise service owners rather than duplicated by each application team:

- Enterprise logging policy and event taxonomy. The organisation defines common event categories, mandatory enterprise sources, severity or priority conventions, minimum record content, time requirements, retention, access, privacy handling, escalation and review responsibilities.

- Corporate identity and authentication logging. Enterprise identity, MFA and directory services record relevant sign-in attempts, authentication failures, account status, authenticator lifecycle, privileged-role assignment and other identity events within their service boundaries.

- Microsoft Windows EUC logging. Hardened corporate Windows devices generate centrally managed operating-system, security, endpoint-protection, device-compliance and software-management events. Application teams do not need to reproduce generic device telemetry in application logs.

- VPN and remote-access logging. Enterprise VPN gateways record connection attempts, authentication outcome, device-compliance result, assigned session details, connection and disconnection times, and relevant security-policy events.

- Network and boundary logging. Enterprise firewalls, routers, load balancers, proxies, DNS, network security appliances and segmentation controls record selected connection, policy, administrative and threat events according to corporate standards.

- Hosting, operating-system and managed-platform logging. Enterprise server, virtualisation, storage, database, middleware, messaging and backup services generate infrastructure and administrative events for the components they manage.

- Privileged-access logging. Enterprise PAM and management services record privileged credential use, session initiation and termination, administrative actions and other relevant events for infrastructure and integrated applications.

- Central collection and analysis. Enterprise log-management and SIEM services receive supported sources, normalise and correlate events, monitor ingestion health, restrict access, apply retention and support SOC investigation, alerting and incident response.

- Enterprise time synchronisation. Approved time services provide a consistent time source for corporate systems so that events from different tiers and services can be correlated reliably.

The application owner must identify the enterprise log sources inherited by the application and must not duplicate them without a defined analytical need. Application logs must complement enterprise telemetry by recording the application context that infrastructure services cannot infer.

### Minimum application-specific event-logging requirements

Each application must implement the following sensible minimum for application-controlled functions, services, interfaces and data:

1\. Maintain an application event-logging matrix. Identify the event category, source component, business or security purpose, trigger, required fields, severity or priority, destination, retention reference, review owner and any sensitive-data restrictions. The matrix must distinguish mandatory events from optional diagnostic logging.

2\. Record authentication and session events within the application boundary. Log successful and failed application authentication where the application performs or receives the result, session creation and termination, significant reauthentication, session rejection, and security-relevant session invalidation. Do not duplicate complete enterprise identity records when a reliable reference or correlation identifier is sufficient.

3\. Record authorisation and access-control events. Log denied access, attempts to use unauthorised roles or functions, access to privileged or high-impact functions, and material changes to roles, permissions, delegations, approval authority and separation-of-duties configuration. Routine successful reads need not all be logged unless required by information sensitivity, law, contract or risk.

4\. Record account, service-identity and authenticator changes controlled by the application. Include creation, enablement, disablement, deletion, role assignment, credential or certificate change, emergency-account use and relevant recovery or reset actions. Secret values must never be written to logs.

5\. Record security-relevant administrative and configuration activity. Log changes to application security settings, logging configuration, trusted endpoints, integration configuration, encryption or key references, retention rules, feature flags with security impact, and production deployment or rollback events where the application can observe them.

6\. Record high-risk business and information events. Identify and log events such as approval, release, cancellation, override, bulk update, bulk extraction, export, restricted-report generation, sensitive-file download, record deletion, retention override and other actions whose misuse could create material harm. The selection must reflect the application threat model and business process.

7\. Record interface, service and batch-processing events. Log security-relevant API, message, file-transfer, scheduled-job and integration activity, including rejected calls, schema or signature failures, replay or duplicate detection, unexpected source or destination, material processing failure and privileged service-to-service action. Avoid recording complete payloads by default.

8\. Record security-control and integrity events. Include significant input-validation failures, malware or prohibited-file outcomes where visible to the application, integrity-check failure, certificate or token validation failure, attempted bypass of security functions, logging disablement, and failure of a required protective service.

9\. Record material application errors without turning ordinary debugging into permanent audit logging. Capture failures that affect security, availability, transaction integrity or evidence, while keeping verbose debug or trace logging disabled in production unless temporarily authorised, time-limited and protected.

10\. Include sufficient, consistent event content. As applicable, records must include a reliable timestamp, event type, source component, acting user or service identity, source context, target object or function, action, outcome, reason or error category, environment, and session, transaction or correlation identifier. Use stable identifiers rather than ambiguous display names where possible.

11\. Enable cross-tier correlation. Browser, thick-client, presentation, application, API, service, messaging and data tiers must propagate or generate suitable correlation identifiers so that a security-relevant transaction can be followed across components. Correlation identifiers must not themselves be reusable authenticators or expose restricted content.

12\. Minimise sensitive content. Logs must not contain passwords, private keys, complete tokens, recovery codes, unmasked payment or identity values, unnecessary personal data, unrestricted request or response bodies, or restricted record content unless expressly required and protected. Record identifiers, categories, hashes or masked values instead where they meet the analytical need.

13\. Send events to the approved destination. Security-relevant production events must be forwarded to the approved central log or SIEM service where technically supported. Local buffering may be used to withstand temporary connection loss, but it must be bounded, protected and forwarded when service resumes. A thick client must not rely on a user-controlled local log as the sole audit record for server-side activity.

14\. Fail and degrade safely. A temporary central logging outage must not silently disable local event generation. The application must define buffering, back-pressure, alerting and recovery behaviour appropriate to risk. Failure to log a critical security or accountable transaction must result in a defined response, which may include rejecting the operation where reliable evidence is essential.

15\. Protect logging configuration and records. Only authorised administrative roles may change event selection, severity, routing or diagnostic level. Application users and ordinary service identities must not be able to alter or delete security records. Application-level access must align with enterprise log protection and retention controls.

16\. Test and review event coverage. Before production use and after material changes, perform representative successful, failed, denied, privileged and error scenarios; verify that the correct events and fields are produced, transported, parsed, time-aligned and usable for investigation; and confirm that sensitive information is not exposed. Review the logging matrix periodically with security, operations, privacy and records stakeholders.

### Proportionate implementation guidance

A small internal application may use a concise logging matrix and a modest set of structured events covering authentication context, denied access, privileged changes, high-impact transactions, exports and application failures. A thick-client application should log local security events only where they add value, forward them when connected, and rely on the receiving service for the authoritative record of server-side transactions. A multi-tier web or service application should use structured records and correlation identifiers consistently across tiers. Applications should prefer fewer high-quality, actionable events over indiscriminate logging that creates cost, privacy risk and investigation noise. Diagnostic detail may be increased temporarily for an approved investigation, but it should have an owner, expiry time and secure handling.

### Expected evidence and assessment artefacts

- Approved application event-logging matrix mapped to business, security, legal, privacy and operational purposes

- Cross-reference to inherited identity, Windows EUC, VPN, network, hosting, database, PAM and SIEM sources

- Representative successful, failed, denied, privileged, configuration, export, interface and integrity event samples

- Event-field and structured-format specification, including correlation and time requirements

- Central forwarding, parsing, routing and retention configuration

- Access-control and change records for logging configuration and application audit data

- Sensitive-data review demonstrating masking, exclusion or approved protection of logged content

- Log-source health, ingestion-delay, parsing and drop or buffer monitoring evidence

- End-to-end test results showing event generation, transport, correlation, analysis and investigation usability

- Periodic event-selection review and approved exceptions or addenda

### Continuous monitoring and measures

- Required application event categories not generated or not received centrally

- Logging sources with stale heartbeats, excessive ingestion delay, dropped events or full local buffers

- Events rejected, unparsed or missing mandatory fields and correlation identifiers

- Material time drift between application components and enterprise sources

- Security-relevant production actions for which no attributable event can be located

- Passwords, tokens, keys or unnecessary restricted information detected in logs

- Unauthorised logging disablement, severity reduction, routing change or record deletion

- Verbose diagnostic logging left enabled beyond its approved period

- Age and outcome of the latest event-selection and end-to-end logging review

### Relationship with related controls

AU-02 determines which event types must be logged and why. AU-03 defines the content of audit records; AU-04 addresses storage capacity; AU-05 addresses response to logging failures; AU-06 governs review, analysis and reporting; AU-08 provides time stamps; AU-09 protects audit information; AU-11 governs audit-record retention; and AU-12 governs audit-record generation. SI-04 uses selected events for system monitoring, IR-05 uses them for incident monitoring, CA-07 uses them for continuing assurance, and SI-12 governs broader information management and retention. Logging every possible action is not the objective: the application must generate the events needed for accountability and detection while avoiding unnecessary sensitive content and noise.

### Addendum trigger

Use an addendum when a legacy, specialist or supplier-controlled application cannot generate a required event; cannot include reliable identity, time, outcome or correlation information; stores security logs only on a user-controlled endpoint; cannot forward events to the approved central service; cannot protect logs from alteration or deletion; requires full sensitive payloads or restricted content to be logged; cannot meet the approved retention or review requirements; cannot distinguish production audit events from verbose diagnostics; or must omit a high-risk business or administrative event. The addendum must identify the affected component and event category, the standard approach, implemented approach, rationale, risk, compensating detection or evidence, sensitive-data handling, monitoring, remediation or replacement plan, accountable owner, approval and review or expiry date.
...
\[12\] NIST Special Publication 800-92, Guide to Computer Security Log Management (2006); NIST SP 800-92 Revision 1 remains an Initial Public Draft at the date of this master standard.
